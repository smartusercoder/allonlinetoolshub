#!/usr/bin/env node

import fs from 'fs';
import path from 'path';
import { fileURLToPath } from 'url';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const TOOLS_DATA_DIR = path.join(__dirname, 'tools-data');
const TOOLS_DIR = path.join(__dirname, 'src', 'pages', 'tools');

function kebabToPascal(str) {
  return str.split('-').map(part => 
    part.charAt(0).toUpperCase() + part.slice(1)
  ).join('');
}

function toolExists(toolId) {
  const componentName = kebabToPascal(toolId);
  const filePath = path.join(TOOLS_DIR, `${componentName}.tsx`);
  return fs.existsSync(filePath);
}

function generateComponent(tool) {
  const componentName = kebabToPascal(tool.id);
  
  return `import React, { useState, useCallback } from 'react';
import ToolLayout from '@/components/ToolLayout';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Textarea } from '@/components/ui/textarea';
import { Label } from '@/components/ui/label';
import { toast } from 'sonner';
import { Copy, Download, RefreshCw } from 'lucide-react';

const ${componentName} = () => {
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);

  const handleProcess = useCallback(() => {
    if (!input.trim()) {
      toast.error('Please enter some input');
      return;
    }
    setIsProcessing(true);
    try {
      // TODO: Implement processing logic
      const result = input;
      setOutput(result);
      toast.success('Processing complete!');
    } catch (error) {
      toast.error('Error processing input');
    } finally {
      setIsProcessing(false);
    }
  }, [input]);

  const handleCopy = useCallback(() => {
    if (output) {
      navigator.clipboard.writeText(output);
      toast.success('Copied to clipboard!');
    }
  }, [output]);

  const handleClear = useCallback(() => {
    setInput('');
    setOutput('');
  }, []);

  return (
    <ToolLayout
      title="${tool.title}"
      description="${tool.desc}"
    >
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Input</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <Label htmlFor="input">Enter your content</Label>
              <Textarea
                id="input"
                placeholder="Enter your content here..."
                value={input}
                onChange={(e) => setInput(e.target.value)}
                className="min-h-[200px] mt-2"
              />
            </div>
            <div className="flex gap-2">
              <Button onClick={handleProcess} disabled={isProcessing || !input.trim()}>
                {isProcessing ? 'Processing...' : 'Process'}
              </Button>
              <Button variant="outline" onClick={handleClear}>
                <RefreshCw className="h-4 w-4 mr-2" />
                Clear
              </Button>
            </div>
          </CardContent>
        </Card>

        <Card>
          <CardHeader>
            <CardTitle>Output</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            <Textarea
              readOnly
              placeholder="Output will appear here..."
              value={output}
              className="min-h-[200px]"
            />
            <div className="flex gap-2">
              <Button variant="outline" onClick={handleCopy} disabled={!output}>
                <Copy className="h-4 w-4 mr-2" />
                Copy
              </Button>
            </div>
          </CardContent>
        </Card>
      </div>
    </ToolLayout>
  );
};

export default ${componentName};
`;
}

// Load tools from JSON
const files = fs.readdirSync(TOOLS_DATA_DIR).filter(f => f.endsWith('.json'));
let allTools = [];

for (const file of files) {
  const content = fs.readFileSync(path.join(TOOLS_DATA_DIR, file), 'utf-8');
  const tools = JSON.parse(content);
  allTools = allTools.concat(tools);
}

console.log(`Loaded ${allTools.length} tool definitions`);

// Generate components
let created = 0;
let skipped = 0;

for (const tool of allTools) {
  if (toolExists(tool.id)) {
    skipped++;
    continue;
  }
  
  const componentName = kebabToPascal(tool.id);
  const filePath = path.join(TOOLS_DIR, `${componentName}.tsx`);
  const content = generateComponent(tool);
  
  fs.writeFileSync(filePath, content);
  created++;
  console.log(`Created: ${componentName}.tsx`);
}

console.log(`\nCreated ${created} components, skipped ${skipped} existing`);
console.log(`Total components: ${fs.readdirSync(TOOLS_DIR).filter(f => f.endsWith('.tsx')).length}`);

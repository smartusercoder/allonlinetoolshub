import { useState } from "react";
import { ToolLayout } from "@/components/ToolLayout";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { Card } from "@/components/ui/card";
import { Copy } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { UsageGuide } from "@/components/UsageGuide";

export default function HtmlBeautifier() {
  const [input, setInput] = useState("");
  const [output, setOutput] = useState("");
  const { toast } = useToast();

  const beautify = () => {
    try {
      // Simple HTML beautifier
      let formatted = input
        .replace(/></g, '>\n<')
        .replace(/\s+/g, ' ')
        .trim();
      
      setOutput(formatted);
      toast({
        title: "Success",
        description: "HTML beautified successfully",
      });
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to beautify HTML",
        variant: "destructive",
      });
    }
  };

  const copyOutput = () => {
    navigator.clipboard.writeText(output);
    toast({
      title: "Copied!",
      description: "Beautified HTML copied to clipboard",
    });
  };

  return (
    <ToolLayout
      title="HTML Beautifier"
      description="Format and beautify HTML code"
    >
      <div className="space-y-6">
        <UsageGuide
          steps={[
            "Paste your minified or messy HTML code",
            "Click \"Beautify HTML\" to format it",
            "Formatted HTML appears with proper indentation",
            "Copy the beautified output"
          ]}
          tips={[
            "Makes minified HTML readable again",
            "Great for debugging compressed HTML",
            "Useful for reviewing third-party code",
            "Helps maintain consistent code formatting"
          ]}
        />
      </div>
      <Card className="p-6">
        <div className="space-y-6">
          <div>
            <label className="block mb-2 text-sm font-medium">Input HTML</label>
            <Textarea
              value={input}
              onChange={(e) => setInput(e.target.value)}
              placeholder="Paste minified HTML..."
              rows={10}
              className="font-mono text-sm"
            />
          </div>

          <Button onClick={beautify} className="w-full">
            Beautify HTML
          </Button>

          {output && (
            <div className="space-y-2">
              <label className="block text-sm font-medium">Beautified HTML</label>
              <Textarea
                value={output}
                readOnly
                rows={10}
                className="bg-muted font-mono text-sm"
              />
              <Button onClick={copyOutput} variant="outline" className="w-full">
                <Copy className="w-4 h-4 mr-2" />
                Copy Output
              </Button>
            </div>
          )}
        </div>
      </Card>
    </ToolLayout>
  );
}
